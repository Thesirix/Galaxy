"""Galaxy — a procedurally generated endless runner built with Kivy."""
...
from kivy.graphics import Color, Line, Triangle, Quad

from transforms import TransformMixin
from user_actions import InputHandlerMixin

Builder.load_file("menu.kv")


class MainWidget(RelativeLayout, TransformMixin, InputHandlerMixin):
    """Main game widget: owns the render loop, game state, and audio."""
    ...
    def init_audio(self):
        """Load all sound assets and configure their playback volumes."""
    ...
    def reset_game(self):
        """Reset all game state variables for a new or restarted run."""
    ...
    def is_desktop(self):
        """Return True when running on a desktop OS with keyboard support."""
    ...
    def check_ship_collisions(self):
        """Return True if the ship overlaps any tile in the current path."""
        for i in range(0, len(self.tiles_coordinate)):
            ti_x, ti_y = self.tiles_coordinate[i]
            if ti_y > self.current_y_loop + 1:
                return False
            if self.check_ship_colision_with_tile(ti_x, ti_y):
                return True
        return False

    def check_ship_colision_with_tile(self, ti_x, ti_y):
    ...
    def init_tiles(self):
        with self.canvas:
            Color(1, 1, 1)
            for _ in range(len(self.tiles_coordinate), self.NB_TILES):
    ...
    def generate_tiles_coordinate(self):
        """Extend the tile path to always have NB_TILES ahead of the player."""
        last_x = 0
        last_y = 0
    ...
    def update(self, dt):
        """Main game loop: advance physics, check collisions, trigger game-over."""
        time_factor = dt * 60
    ...
    def on_menu_button_pressed(self):
        """Handle start / restart button: reset state and begin the game."""
        if self.state_game_over:
...
class GalaxyApp(App):
    """Kivy application entry point for the Galaxy endless runner."""
    pass
